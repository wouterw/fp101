...
-- doubleSecond xs = zipWith ($) (cycle [id,(*2)])

-- doubleSecond ns = zipWith (\x y -> if even x then y * 2 else y) [1 .. length ns + 1] ns
...
-- sumDigits ns = foldr (\x y -> y + (sum . toDigits $ x)) 0 ns
...
isValid 0 = False
isValid x = 0 == (sumDigits (doubleSecond (toDigitsRev x)) `mod` 10)
